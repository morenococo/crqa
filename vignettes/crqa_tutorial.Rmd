...
Rcpp inner loop that never materialises the N×N distance matrix. Figure 1
shows wall time (left panel) and peak memory (right panel) measured on
the Rössler-system benchmark of Marwan & Kraemer (2023, *EPJST* 232:5–27)
— the standard reference for RQA package comparisons (m = 3, τ = 6,
Euclidean norm, 30 s cutoff per call). Solid lines are measured on this
hardware; reference levels for other packages are from the paper or
derived from each implementation's algorithmic memory requirements.

```{r benchmark-fig, echo = FALSE, out.width = "100%", fig.cap = "Figure 1. Speed (left) and peak memory (right) for crqa v2.1.0 vs. other RQA implementations on the Rössler-system benchmark (Marwan & Kraemer 2023). Solid lines: measured. Dashed lines: reference levels from the paper or derived from algorithm characteristics. crqa v2.0.7 (legacy) hits the 30 s wall and OOMs at N ≈ 12 k. crqa v2.1.0 (fused) is 5–15x faster than v2.0.7 and comparable to pyunicorn on CPU. crqa v2.1.0 (aRQA) — available as method='aRQA' in this package — extends to N = 200 000 in ~6 s with O(N) memory."}
knitr::include_graphics("figure3_speed_memory.png")
```

`crqa v2.1.0 (fused)` is 5–15× faster than v2.0.7 and comparable to
pyunicorn (Cython, Python) on CPU. The approximative path
(`method = "aRQA"`) — **part of this package** — scales to N = 200 000
in ~6 s, a regime where all exact O(N²) methods run out of memory.
Unlike specialised GPU libraries (AccRQA, PyRQA), `crqa` also provides
cross-recurrence, multidimensional analysis, windowed methods, and
parameter optimisation — features unavailable in those tools.

The fused kernel is also **OpenMP-parallel** (new in v2.1.0). On
multi-core systems it scales near-linearly with cores at typical
sparsity (RR ≲ 1 %) and gives ~2× speedup on 4 cores beyond the figures
above. crqa respects `OMP_NUM_THREADS` — set it to `1` to force serial
execution, or to `N` to cap parallelism. When using a wrapper's
`workers` argument, set `OMP_NUM_THREADS = 1` in each worker to avoid
